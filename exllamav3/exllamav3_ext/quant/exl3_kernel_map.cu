...
int select_gemm_shape(int cc, int size_m, int size_k, int size_n, int bits)
{
    ...
    switch(cc)
    {
        ...
        case CC_BLACKWELL:
            ...
            if (size_n <= 2048 && size_k > 4096 && bits >= 5) return 1;
            if (size_n <= 4096) return (size_k && bits >= 3) > 8192 ? 3 : 2;
            if (mod_512 && size_n > 16384 && bits >= 6) return 4;
            if (mod_256 && bits >= 3) return 3;
            return 2;
    }
    return 0;
}
...
)
{
    ...
    if (out_block_dim) *out_block_dim = EXL3_GEMM_BASE_THREADS * exl3_gemm_tilesize_k[shape_idx] / 16;
