...
)
{
    ...
    int count = B.numel();
    int num_sms = force_num_sms;
    TORCH_CHECK(num_sms > 0 && num_sms <= total_sms, "invalid BF16 MGEMM SM count");
    int concurrency = std::min(total_sms / num_sms, count);
...
)
{
    ...
    TORCH_CHECK(C_scratch.size(1) == A.size(0));
    TORCH_CHECK(C_bf16.dim() == 2 && C_bf16.size(0) == A.size(0));
...
)
{
    ...
    int size_n = C_scratch.size(2);
    int num_sms = force_num_sms;
    TORCH_CHECK(num_sms > 0 && num_sms * count <= total_sms,
                "grouped-Had BF16 MGEMM requires all matrices resident");
    int* locks_ptr = DevCtx::instance().get_locks(device);
    cudaStream_t stream = at::cuda::getCurrentCUDAStream().stream();
...
)
{
    ...
    check_cuda_tensor(A_had, A, "A_had");
    check_cuda_tensor(svh, A, "svh");
    check_cuda_tensor(had_group_ids, A, "had_group_ids");
    TORCH_CHECK_DTYPE(A, kBFloat16);
    TORCH_CHECK_DTYPE(B, kLong);
    TORCH_CHECK_DTYPE(C_scratch, kHalf);
    TORCH_CHECK_DTYPE(C_bf16, kBFloat16);
    ...
    if (bits == 5) return launch_mgemm_bf16_io_grouped_had<5, 1>(A, B, C_scratch, C_bf16, unique_suh, A_had, svh, had_group_ids, force_num_sms, output_stride, direct_output, final_group_barrier);
    if (bits == 6) return launch_mgemm_bf16_io_grouped_had<6, 1>(A, B, C_scratch, C_bf16, unique_suh, A_had, svh, had_group_ids, force_num_sms, output_stride, direct_output, final_group_barrier);
