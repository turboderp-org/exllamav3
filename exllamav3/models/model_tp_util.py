...
def _cuda_host_register(ptr: int, nbytes: int, flags: int = 0):
    torch.cuda.synchronize()
    cudart = _cudart()
    fn = cudart.cudaHostRegister
    ...
    if err != CUDA_SUCCESS:
        raise RuntimeError(f"cudaHostRegister({hex(ptr)}, {nbytes}) failed: {err}")


def _cuda_host_unregister(ptr: int):
    torch.cuda.synchronize()
    cudart = _cudart()
    fn = cudart.cudaHostUnregister
    fn.argtypes = [ctypes.c_void_p]
    fn.restype = ctypes.c_int
    err = fn(ctypes.c_void_p(ptr))
    if err != CUDA_SUCCESS:
        raise RuntimeError(f"cudaHostUnregister({hex(ptr)}) failed: {err}")
