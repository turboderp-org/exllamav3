...
class BlockSparseMLP(BlockSparseMLP_CPU, Module):
    ...
    def forward(
        ...
        # single-expert kernels and the bszN graph) reads the expert trellis weights
        ...
        # path. The bszN graph may also embed the shared experts (bc_sh_exp) and the
        ...
        # Torch/C++/fused path
            ...
            if self.num_local_experts is None or self.num_local_experts > 0:
                ...
                # (experts_lora: the fused kernel reads the base trellis only; take the
                # per-expert loop so the torch path can apply the adapter)
                if (
                    self.fused_mode_buffers is not None and not experts_lora and
                    num_tokens * top_k <= TEMP_ROWS_FUSED
                ):
                    run_fused(-1)
        ...
        # (sh_fused_lora: the graph may embed the shared experts/gate, which never see a
        # runtime LoRA -- skip to the mgemm/single paths below and run them unfused instead)
        elif bszn_eligible and not sh_fused_lora:
