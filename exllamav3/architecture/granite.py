...
class GraniteConfig(Config):
    arch_string = "GraniteForCausalLM"
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            directory,
            derived_model if derived_model else {"text": GraniteModel},
            **kwargs
        )
        ...
        self.residual_multiplier = self.read_cfg(float, "residual_multiplier", None)
        self.logits_scaling = self.read_cfg(float, "logits_scaling", 1.0)
...
class GraniteModel(Model):
        ...
        **kwargs
        ...
        self.modules += [
            ...
            Linear(
                ...
                post_scale = 1.0 / config.logits_scaling,
            )
        ]
    ...
    def default_chat_prompt(self, prompt: str, system_prompt: str = None) -> str:
        p = ""
        if system_prompt:
            p += f"<|start_of_role|>system<|end_of_role|>{system_prompt}<|end_of_text|>\n"
        p += f"<|start_of_role|>user<|end_of_role|>{prompt}<|end_of_text|>\n"
        p += f"<|start_of_role|>assistant<|end_of_role|>"
        return p
